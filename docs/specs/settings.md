...
- **布局**：设置页分三区——偏好区（主题/locale/displayTimezone）、AI 区（默认关闭：开关/端点/Key/模型/连接测试/隐私说明/发送预览开关/清除缓存，见 [AI 功能规格](ai-features.md) §4.2）、数据区（导出备份 / 导入备份 / 系统重置）。各区以 `border-t` 分隔，不用嵌套卡片。
