...
- 键长均为 6，无前缀包含关系歧义（`044010` vs `044120` 第 3 位即不同），精确 6 字符查表即可。
- `F44010` 为**字符串**前缀（含字母），非纯数字；查表前对前缀做一次 `toUpperCase()`，
  兼容小写/全角之外的偶然大小写差异（数字不受影响）。
- 馆名直接作为领域数据返回，不做 i18n 翻译（同 `classification.ts` 表约定：类目名/馆名是领域事实）。
