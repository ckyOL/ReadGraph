...
- 2026-08-13 **OPAC 补全交互重构（详情页逐本工作流，用户定案）已落地**：放弃列表批量补全（批量全量/勾选驱动/会话 store 三案否决，存档 [opac-enrichment §10.1](../specs/opac-enrichment.md#101-存档被否决的批量方案)）——书库列表与导入页**无任何批量补全入口**；补全唯一路径 = 详情页单条（每编目卡各自 provider 按钮，来源天然隔离）+ 头部「‹ 上一个 / 下一个 ›」翻页（沿列表视图顺序，`library-view.ts` 共享派生 + 详情页 search 扩展视图参数）；抓取上下文为**详情页组件态**（取消保留、重抓覆盖、保存后 onSaved 清除、刷新丢失=重抓）；编辑 Dialog 目标编目高亮 + provider 徽标（`OpacProvider.shortName`）溯源。删除：`enrich-session.ts`、`enrich-batch.tsx`、批量编排/候选集聚合函数（`enrichCatalogRecords`/`collectCandidates`/`groupCandidatesByProvider`/`countPlaceholderExcluded`）及 `enrich.*` batch i18n 键。落地：`src/lib/library-view.ts`（buildLibraryRows/filterAndSortRows/adjacentBookIds + 12 单测）、`index.tsx` 行链接带视图参数、`$bookId.tsx` 翻页 + 组件态上下文、`-edit-dialog.tsx` 目标高亮/provider 徽标/onSaved、e2e pager 用例（视图顺序遍历 + 边界禁用 + 列表无批量入口 + 详情页补全按钮可见）。验证：Vitest 577 绿 + `pnpm build` + Playwright 37 全绿。

- 2026-08-04 导入报告警告列表改分组布局：单行 flex（徽章+消息+等宽引用抢宽）在 320px 侧栏把消息压到 ~0px 导致 CJK 逐字折行 → 按类型归组（组头徽章 + 等宽 `×N` 计数，细罫线分隔），消息独占整行 `break-words`，`recordRef` 降级为条目底部等宽弱化标签行（超长 `truncate` + `title`），标题带总数（`import.report.warnings_{one,other}`），`format_error` 组徽章用 destructive 变体区分错误；归组纯函数 `src/import/warning-groups.ts` + Vitest 3 例（TDD Red→Green），新增 `import.warning.count`/`import.report.warnings_*` i18n 键（zh-CN/en 同步）；真实 bugfile（7109377↔7109378 多卷候选）浏览器验证：5 月单条、6 月 8 条均短语级正常折行，无逐字断行。

- 2026-08-02 阅读画像改 Tabs 布局：书多时甘特固定 320px 视口内 lane 被压扁（「显示不全」）→ 四图改为 shadcn `Tabs` 独占全幅（分类法分布 / 借阅甘特带 / 借阅量 / 借阅时长分布，标签复用 `profile.chart.*.title`；每次仅挂载一个图块，非激活卸载释放 echarts 实例与定时器）；甘特视口按 lane 数自适应（`src/profile/charts/gantt-viewport.ts` 纯函数：lane 可视高 24px，`clamp(280, 624)`；超可视上限 26 lane 启用右侧 y 轴缩放 slider，默认窗口显示顶部 26 lane，lane 不再被压扁）；treemap 480px / 柱图 360px / 时长分布 320px 视口提升；`ProfileSkeleton` 对齐 Tabs 形态。E2E 改为逐 tab 激活断言单 canvas（profile.spec 5 例全绿，大库用例断言自适应高度 > 400px）；Vitest 新增 `gantt-viewport.test.ts` 4 例。[reading-profile 规格 §3/§4/§6/§7](../specs/reading-profile.md#4-ui-设计说明) 同步改写。另修复**既有失败**：`e2e/app.spec.ts` 导入关键路径断言隐私虚构化前的旧标题「合成绘本甲」（夹具 2026-07 已虚构化为合成书目，断言漏改，stash 验证为 main 上既有红）→ 改为「合成书目001」，导入用例恢复绿。`tsc -b` + Vitest 35 suites/358 tests + `pnpm build` + E2E 2…
