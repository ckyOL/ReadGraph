...
- 2026-08-18 **SEC-3 完成**：README.md/README-zh.md 四处「完全离线」声称（L7/L94 双语）修正为「数据离线存储于浏览器 IndexedDB；断网可用性取决于托管方缓存策略；本仓库不提供 Service Worker」；grep `works fully offline|Offline-capable|可完全离线|无需网络` 归零。偏差：纯文档任务无 Red 阶段。
...
- 2026-08-18 **SEC-5 完成**：package.json `version` 0.0.0→1.0.0（lockfile 不变）；新 `CHANGELOG.md`（Keep a Changelog，v1.0.0 2026-08-18 手写——纯前端 SPA/szlib 管线/画像图表/OPAC 补全/CLC 分类/WCAG 2.2 AA/CI/MIT+第三方声明/CSP 与外链加固，含 [Unreleased] 占位）；AGENTS.md 追加「Release」段（版本提升→CHANGELOG→门禁→tag→push→gh release）。偏差：push/tag/release 按协议推迟至 Main 合并验证后执行。
