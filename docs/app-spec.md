...
| 8 | 调试模式规格 | ✅ 已落地 | [specs/debug-mode.md](./specs/debug-mode.md) | **仅 dev 启用**（构建期常量 `__DEBUG_MODE__`：`pnpm dev` 即开，build/vitest 注入 false，生产包 debug 分支剔除且产物零残留；verbose 走 `localStorage['readgraph:debug']`）、DevTools 主通道（`[readgraph:import]` 结构化日志 + `console.table` + performance + 全局钩子）、导入决策 Trace 纯函数数据契约（新增/跳过/合并逐行明细）— 已 TDD 落地（2026-09-07 收尾：1157 单测全绿，生产包 debug 字符串零残留，`pnpm dev` 冒烟 US1–US6 通过）；**不做**应用内调试 UI（DevTools 为唯一观察通道，M1 即闭环） |
