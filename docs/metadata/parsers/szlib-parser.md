...
- `title`、`subtitle`、`parallelTitles`、`authors`、`translators`：由 `src/lib/title.ts` 的 `parseTitle(rawTitle)` 结构化解析派生，具体规则见 [import-pipeline §13 书目标题结构化解析](../../specs/import-pipeline.md#13-书目标题结构化解析)。
  - 解析前先经 `src/lib/encoding.ts` 的 `decodeHtmlEntities` 解码 HTML 实体（OPAC 导出常见 `&apos;`/`&amp;` 等），保证题名/责任者干净且无 ISBN 去重键一致。
  - `Book.title` 存正题名（**含 ` : ` 段，不切分副题名**——深图数据中 ` : ` 语义与丛书分册同构不可辨，切分会使系列各册在书库列表同名，见 [import-pipeline §13 输入形状](../../specs/import-pipeline.md)），不再原样保留原始编目串；原始串由 `RawRecord.data.title` 留存以便溯源与重解析。
...
- 键长均为 6，无前缀包含歧义（`044010` 罗湖 vs `044120` 龙岗第 3 位即不同），精确 6 字符查表即可。
- `F44010` 为**字符串**前缀（含字母 `F`），非纯数字；查表前对前缀做一次 `toUpperCase()` 归一（数字不受影响）。
- 长度不足 6 位 / 未知前缀 / 非深图条码 → 无归属馆（解析为 `null`，UI 不渲染）。
- 馆名是领域数据，不做 i18n 翻译（同 `src/lib/classification.ts` CLC/DDC 表约定）。
