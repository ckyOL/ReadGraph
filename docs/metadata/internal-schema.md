...
const booksStore = {
  ...
  indexes: [
    ...
    { name: 'clcCode', keyPath: 'clcCode', options: { unique: false } },
...
Book 去重（合并规则）:
1. 精确匹配: isbn13 相同 → 合并
2. 条码匹配: 同一 sourceId + barcode → 同一本书的同一副本
3. 模糊匹配: normalize(title) + normalize(authors[0]) → 建议合并（需用户确认）
