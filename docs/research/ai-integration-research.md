...
> 对应到本项目：上下文最小化 = 只发聚合统计 + Top 书目题名/作者（§5 字段白名单）；可自定义 prompt = 画像/点评 prompt 用户可调；引用定位 = 事实洞察锚定聚合维度（点击跳图表 tab）、审美点评锚定画像整体。
...
脱敏保护的是**直接标识符与行为细节**（卡号、条码、借还时点、频率、馆名、原始记录）；**不保护阅读兴趣本身**——审美点评与画像洞察必然发送书名/作者（Top N 题名+作者，§5.2 白名单），品味判断与洞察的趣味性都依赖题名。这是功能语义决定的，无法"脱敏"。因此：
...
|------|-------------------|-------------------|
| 阅读画像洞察 | 聚合统计（分类分布/借阅量/时长均值/年度指标，来自 `profile-stats` 输出）+ Top N 题名+作者 | 同上 + 单条借阅记录、时间戳明细 |
| 年度总结叙事 | `year-review` 切片指标 + Top 书目题名+作者 | 同上 |
...
> **审美点评与洞察共用同一白名单**（聚合统计 + Top N 题名/作者）——点评基于画像级数据，不额外发送单本书目元数据。
...
- **审美情趣点评**：作为画像分析的组成部分，`insight[]` 条目带 `kind`（`'fact'` 事实洞察 / `'taste'` 审美点评）——`kind='taste'` 条目即审美点评：综合借阅书目等信息（聚合统计 + Top 书目题名/作者，§5.2 白名单）生成阅读品味/审美倾向的整体评价（中英随 locale），正文为一段评价性文字，无维度关联。与事实条目**同次生成、同一 Zod schema 强校验**。
- **输入独立性**：生成输入只消费本地聚合与书目（`profile-stats` 输出 + Top 书目题名/作者），**不消费前一次 AI 输出**——防幻觉传播，守住「AI 只转译本地数据」（design-decisions §6）。
