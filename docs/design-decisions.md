...
**人工审核路线（同 ISBN 多卷/占位）**:
- 导入期零决策：同 ISBN 多 metaid 合并（套装候选）与选书帮占位均在导入时仅置 `needsReview=true`，不做自动拆分/合并判断（跨馆题名不可靠，自动判定会误拆）。
- 统一收口到书目详情页 [/library/$bookId 编辑表单](./specs/book-editing.md)：占位补全/套装卷号结构化与普通书目编辑共用同一表单（全字段可改、保存即解除标记）；合并到已有书目、拆为独立 Book 等跨实体操作走详情页「更多」菜单二次确认。**/review 页已废弃删除**：待完善发现与过滤由书库列表承载（占位/套装徽标 + 类型筛选 + 计数徽标，见 [book-editing §5](./specs/book-editing.md#5-书库列表承载待完善标记与过滤)）；待审类型语义见 [book-editing §10](./specs/book-editing.md#10-待审类型语义并入自原-reviewmd)。
...
### 6. 补全入口收敛：详情页逐本工作流（2026-08-13 定案）

**决策**: OPAC 编目补全**不做列表级批量**——书库列表/导入页无任何批量补全入口；补全唯一路径是详情页单条（每条 `CatalogRecord` 卡各自 provider 的补全按钮），详情页头部加「‹ 上一个 / 下一个 ›」翻页（沿书库列表当前筛选/排序视图），补全 → 编辑（同页 Dialog）→ 保存 → 翻下一本，全程不离开详情页上下文。抓取建议上下文为**详情页组件态**（不落 URL、不落库、不引入 store/sessionStorage），取消保留、重抓覆盖、刷新丢失（重抓一次即可）。

**原因**:
- **编目粒度**：列表行 = Book（可含多来源多编目），批量抓取在编目粒度上无从表达——勾选校验/来源分组/目标编目识别整套复杂度只为「一次抓多本」服务。
- **来源隔离**：一个补全按钮 = 一个 provider = 一条编目，多来源「乱套」从入口上消灭。
- **状态生命周期**：批量结果面板跨页保留、跨刷新恢复等全部需求随批量入口删除而消失；组件态覆盖单条场景，无过度设计。

**约束**:
- 已删除：`enrich-session.ts`（单槽与会话 store 两版）、`enrich-batch.tsx`（批量面板）、`enrich-service` 批量编排与候选集聚合（`enrichCatalogRecords`/`collectCandidates`/`groupCandidatesByProvider`/`countPlaceholderExcluded`）。
- 详情页 `search` 扩展列表视图参数（`q`/`source`/`status`/`sort`/`dir`），翻页派生逻辑抽 `src/lib/library-view.ts` 与列表页共用（单一实现，防漂移）。
- 历史否决记录（批量全量 / 勾选驱动 / 会话 store）见 [opac-enrichment §10.1](./specs/opac-enrichment.md#101-存档被否决的批量方案)。
...
## 性能预算与 PWA（2026-08 CWV 基线与 Service Worker 评估裁定沉淀）

- **性能预算以真实部署场景为准**：本项目为纯前端本地部署（localhost/局域网宽带 +
  桌面 CPU）。实测 desktop 宽带预设 LCP 0.7s、性能分 1.0；本机不节流 LCP 0.2s——
  **均达标**（CWV 预算 LCP ≤ 2.5s / INP ≤ 200ms / CLS ≤ 0.1）。Lighthouse "Fast 3G"
  移动预设（3.9s）是移动托管场景的保守参考：公网托管 + 手机弱信号目标场景若立项
  （物理下限 ≈2.8s，需 SSR/静态 shell 预渲染，超出纯前端范围），性能与 PWA 捆绑评估，
  不单独优化。
- **不提供 Service Worker / 不做 PWA**（2026-08-20 评估定案）：数据层（IndexedDB）已
  离线，SW 仅能补 app shell，而断网刷新问题在本地部署下不存在；静态资源与数据同机，
  SW 无收益面。README 对外声明「数据离线存储于浏览器 IndexedDB；断网可用性
  取决于托管方缓存策略；本仓库不提供 Service Worker」。触发重评条件 = 上述公网托管
  移动场景立项。
