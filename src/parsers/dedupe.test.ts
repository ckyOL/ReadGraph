...
describe('dedupeBorrowCycles', () => {
  ...
  it('精确重复（sourceId+barcode+borrowedAt）跳过，记 duplicate 警告', () => {
    ...
    const r = dedupeBorrowCycles(
      [{ sourceId: 'szlib', barcode: 'B1', bookId: 'bk', borrowedAt: t, returnedAt: t2, status: 'returned', borrowLocation: null, returnLocation: null, rawRecordIds: ['r2'] }],
      existing,
    )
  ...
  it('批次内重复：同秒空条码不同 bookId 的两条候选均保留（bookId 身份分量）', () => {
    ...
    const r = dedupeBorrowCycles(
      [
        { ...base, bookId: 'bk-A' },
        { ...base, bookId: 'bk-B', rawRecordIds: ['r3'] },
      ],
      [],
    )
    expect(r.skippedFlags).toEqual([false, false])
    expect(r.cycles).toHaveLength(2)
    expect(r.cycles.map((c) => c.bookId)).toEqual(['bk-A', 'bk-B'])
  ...
  it('既有周期同键（sourceId+barcode+borrowedAt）不同 bookId → 新候选不跳过（bookId 身份分量）', () => {
    ...
    const r = dedupeBorrowCycles(
      [{ sourceId: 'szlib', barcode: null, bookId: 'bk-B', borrowedAt: t, returnedAt: t2, status: 'returned', borrowLocation: null, returnLocation: null, rawRecordIds: ['r2'] }],
      existing,
    )
