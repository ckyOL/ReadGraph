...
- **导入与去重** — 单屏导入向导；支持多来源 JSON 导出（深圳图书馆 OPAC 为首个实现）；编码自动检测；跨文件合并，按 条码 → ISBN → 题名+作者 依次去重。
...
- **阅读画像** — 统计与 ECharts 可视化：阅读节奏、时间线、矩形树图、借阅日历热力图、消费金额与价格分布；电子书阅读器等设备借阅单独记录并从统计中排除。
- **年度回顾** — 专属年度视图：年度目标、最常借 Top N、可选 AI 年度叙事，并可生成可下载/分享的年度报告图（3:4 与 9:16 两种比例）。
- **AI 解读（可选）** — 自带 OpenAI 兼容端点（云端 BYOK 或本地 Ollama / LM Studio），默认关闭；白名单脱敏 + 发送预览严格控制外发内容，结果以 Markdown 流式呈现。
- **设置** — 时区城市选择（tzdb）、中英双语界面、备份导出/恢复、整体重置。
...
| 图表 | ECharts |
| 并发 | Web Worker + Comlink — 导入管线与统计不阻塞主线程 |
...
| [docs/research/](./docs/research/) | 技术调研笔记：AI 集成路线、年度分享图、Bookology 对标分析 |
...
- **纯前端** — 所有数据存于浏览器 IndexedDB，不上传任何服务器；仅当显式启用可选 AI 解读（BYOK）时，才会把经白名单脱敏的统计摘要发送到你自行配置的端点。
